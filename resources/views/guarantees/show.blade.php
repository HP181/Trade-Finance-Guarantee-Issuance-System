                        ...
                        <div class="row mb-4">
                            <div class="col-md-12">
                                ...
                                <table class="table table-bordered">
                                    <tr>
                                        <th>Review Notes:</th>
                                        <td>{{ $guarantee->review->review_notes ?: 'No notes available' }}</td>
                                    </tr>
                                    <tr>
                                        <th>Reviewed By:</th>
                                        <td>{{ $guarantee->review->reviewer ? $guarantee->review->reviewer->name : 'Not yet reviewed' }}</td>
                                    </tr>
                                    <tr>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="d-flex gap-2">
                                @if ($guarantee->isDraft() && $guarantee->user_id == Auth::id())
                                    <form method="POST" action="{{ route('guarantees.submit-for-review', $guarantee->id) }}" class="me-2">
                                        @csrf
                                        <button type="submit" class="btn btn-info" onclick="return confirm('Are you sure you want to submit this guarantee for review?')">
                                            Submit for Review
                                        </button>
                                    </form>
                                @endif
                                ...
                                @if (Auth::user()->isAdmin() && $guarantee->isApplied())
                                    <form method="POST" action="{{ route('guarantees.issue', $guarantee->id) }}" class="me-2">
                                        @csrf
                                        <button type="submit" class="btn btn-success" onclick="return confirm('Are you sure you want to issue this guarantee?')">
                                            Issue Guarantee
                                        </button>
                                    </form>
                                ...
                                @if (($guarantee->isDraft() || $guarantee->isRejected()) && (Auth::user()->isAdmin() || $guarantee->user_id == Auth::id()))
                                    <form method="POST" action="{{ route('guarantees.destroy', $guarantee->id) }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this guarantee? This action cannot be undone.')">
                                            Delete Guarantee
                                        </button>
                                    </form>
                                @endif
